...
public interface StudentFollowedCourseRepository extends JpaRepository<StudentFollowedCourseEntity, Long> {
    ...
    @Query("SELECT new individualassignment.edubridge.domain.users.DateCountFollowingStudents(s.followingDate, COUNT(s.student)) FROM StudentFollowedCourseEntity s WHERE s.followingDate BETWEEN :startDate AND :endDate GROUP BY s.followingDate")
    ...
    @Query("SELECT new individualassignment.edubridge.domain.users.DateCountFollowingStudents(s.followingDate, COUNT(DISTINCT s.student)) FROM StudentFollowedCourseEntity s WHERE s.course.id = :courseId AND s.followingDate BETWEEN :startDate AND :endDate GROUP BY s.followingDate")
    ...
    @Query("SELECT s.course FROM StudentFollowedCourseEntity s GROUP BY s.course ORDER BY COUNT(s.student) DESC")
    List<CourseEntity> findMostFollowedCourses(Pageable pageable);
}
